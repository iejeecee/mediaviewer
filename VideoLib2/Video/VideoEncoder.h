...
protected:
	...
	void validateOutputStream(const VideoLib2::Stream *stream) {
		...
		if(stream->getCodec()->pix_fmts != NULL) {
			...
			{
				throw gcnew VideoLib2::VideoLibException("Pixel format not supported for output stream " + 
		...
		if(stream->getCodec()->supported_framerates != NULL) {
			...
			{
				throw gcnew VideoLib2::VideoLibException("framerate not supported for output stream " + stream->getTimeBase().den + "fps");
			}
		}
		...
		if(stream->getCodec()->supported_samplerates != NULL) {
			...
			{
				throw gcnew VideoLib2::VideoLibException("Sample rate not supported for output stream " + stream->getCodecContext()->sample_rate + "hz");
			}
		} 
		...
		if(stream->getCodec()->sample_fmts != NULL) {
			...
			{
				throw gcnew VideoLib2::VideoLibException("Sample format not supported for output stream " +
		...
		if(stream->getCodec()->channel_layouts != NULL) {
			...
			{
				throw gcnew VideoLib2::VideoLibException("Channel layout not supported for output stream " + 
...
public: 
	...
	void open(MemoryStreamAVIOContext *memoryStreamCtx, const char *formatName) {
		...
		if(formatContext == NULL) {
			...
			throw gcnew VideoLib2::VideoLibException("Cannot create output format context with memorystream");
		}
	...
	virtual void open(String ^outputFilename, const char *formatName = NULL) {
		...
		if(formatContext == NULL) {
			...
			throw gcnew VideoLib2::VideoLibException("Cannot create output format context: " + outputFilename);
		}
	...
	Stream *createStream(Stream *copy) {
		...
		if(encoder == NULL) {
			...
			throw gcnew VideoLib2::VideoLibException("Could not find suitable encoder for stream: " + marshal_as<String ^>(formatContext->filename));
		}
		
		AVStream *stream = avformat_new_stream(formatContext, encoder);
		if (stream == NULL) {
			
			throw gcnew VideoLib2::VideoLibException("Could not create output stream for: " + marshal_as<String ^>(formatContext->filename));
		}
		
		int ret = avcodec_copy_context(stream->codec, copy->getCodecContext());
		if (ret != 0) {

			throw gcnew VideoLib2::VideoLibException("Copying stream context failed: " + marshal_as<String ^>(formatContext->filename) + " ", ret);							
		}
		...
		VideoLib2::Stream *newStream = new VideoLib2::Stream(stream, encoder);
	...
	Stream *createStream(const std::string &encoderName, int width, int height, const AVRational &sampleAspectRatio,
		const AVRational &timeBase, Dictionary<String ^, Object ^> ^encoderOptions, AVPixelFormat pixelFormat = AV_PIX_FMT_NONE) 
	{
		_ASSERT(formatContext != NULL);
		...
		if(pixelFormat == AV_PIX_FMT_NONE) {
			...
			if(encoder->pix_fmts == NULL) {
				...
				throw gcnew VideoLib2::VideoLibException("No default pixel format for output stream");
			}
		...
		if(encoderOptions != nullptr) {
			...
			while(enumerator->MoveNext()) 
			{
				...
				newStream->setOption(marshal_as<std::string>(pair->Key), marshal_as<std::string>(pair->Value->ToString()));
			}
	...
	Stream *createStream(const std::string &encoderName, int sampleRate, int nrChannels, Dictionary<String ^, Object ^> ^encoderOptions) 
	{
		_ASSERT(formatContext != NULL);
	...
	void writeHeader() {
		...
		if(formatContext->pb == NULL) {
			...
			if (ret < 0) {
				...
				throw gcnew VideoLib2::VideoLibException("Could not open output file: " + marshal_as<String ^>(formatContext->filename) + " ", ret);										
			}
		}
		...
		if (ret < 0) {
			...
			throw gcnew VideoLib2::VideoLibException("Error writing header: " + marshal_as<String ^>(formatContext->filename) + " ", ret);	
	...
	bool encodeFrame(int streamIdx, const AVFrame *frame, AVPacket *encPacket) {
		...
		else if(stream[streamIdx]->getCodecType() == AVMEDIA_TYPE_AUDIO) 
		{
			...
			throw gcnew VideoLib2::VideoLibException("Trying to encode non audio/video frame: " + marshal_as<String ^>(formatContext->filename));	
		}

		if(ret < 0) {

			throw gcnew VideoLib2::VideoLibException("Error encoding frame: " + marshal_as<String ^>(formatContext->filename) + " ", ret);	
		}
	...
	void writeEncodedPacket(AVPacket *encPacket) {
		...
		if(ret != 0) {
			...
			throw gcnew VideoLib2::VideoLibException("Error writing encoded packet for: " + marshal_as<String ^>(formatContext->filename) + " ", ret);	
		}
	...
	void writeTrailer() {
		...
		if(ret != 0) {
			...
			throw gcnew VideoLib2::VideoLibException("Error writing trailer for: " + marshal_as<String ^>(formatContext->filename) + " ", ret);	
		}
	}
